...
def main():
    ...
    algorithm_builders = [
        ...
        ("OPDN", lambda: {p: rating for p, rating in zip(players, one_pass_opdn_stream(
            [(player_to_idx[p1], player_to_idx[p2], 1 if w1 else 0) for p1, p2, w1 in matches], 
            len(players), ridge=1e-2, step_cap=0.1))}),
    ]
