...
def main():
    ...
    ap.add_argument("--n-games", type=int, default=200, help="Use first N games (few-game regime).")
    ...
    ap.add_argument("--show-table", action="store_true", help="Show one run's per-player table for each method.")
    ...
    matches_full = read_matches(args.csv)
    if args.n_games > 0:
        matches = matches_full[:args.n_games]
    else:
    ...
    if args.show_table or args.save_csv:
        # Run each algorithm and show tables or save CSVs
        for name, builder in algorithm_builders:
            ...
            if args.show_table:
                order = sorted(players, key=lambda p: a_hat[p], reverse=True)
                print("\n-------------------------------------------")
                print(f"{name} — One-pass table (subset size = {len(matches)})")
                print("-------------------------------------------")
                print("Rank | Player |   θ (centered)  |   Elo   | Wins")
                print("-------------------------------------------")
                for rk, p in enumerate(order, 1):
                    print(f"{rk:4d} | {int(p):6d} | {a_hat[p]:13.6f} | {elo[p]:7.1f} | {wins_count[p]:4d}")
